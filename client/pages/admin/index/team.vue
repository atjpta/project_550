    ...
    <div v-else>
      <div class="space-y-3 mt-5" v-if="dataPerPage[0]">
        <div v-for="(i, n) in dataPerPage" :key="i._id">
          ...
          <div v-if="n < useReport.list_search_team.length - 1" class="divider"></div>
        </div>
      </div>
      <div v-else>
...
const maxPage = computed(() => {
  selectPage.value = 1;
  return Math.ceil(useReport.list_search_team.length / size);
});
...
const dataPerPage = computed(() => {
  ...
  for (let i = 0; i < size; i++) {
    if (index < useReport.list_search_team.length)
      list.push(useReport.list_search_team[index]);
    index++;
  }
