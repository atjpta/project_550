    ...
    <div v-else>
      <div class="space-y-5 my-5" v-if="dataPerPage[0]">
        <div v-for="(i, n) in dataPerPage" :key="i._id">
          <AdminVMonoTopic :data="i" />
          <div v-if="n <
            (useReport.list_search_topic.length > size
              ? size
              : useReport.list_search_topic.length) -
            1
            " class="divider"></div>
        </div>
      </div>
...
const maxPage = computed(() => {
  selectPage.value = 1;
  return Math.ceil(useReport.list_search_topic.length / size);
});
...
const dataPerPage = computed(() => {
  ...
  for (let i = 0; i < size; i++) {
    if (index < useReport.list_search_topic.length)
      list.push(useReport.list_search_topic[index]);
    index++;
  }
