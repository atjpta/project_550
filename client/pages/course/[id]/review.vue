<template>
  <div class="bg-gradient-to-l from-info/10 via-info/5 to-pink-500/5 rounded-md p-5">
    <!-- phần edit review -->
    <div v-if="openEdit">
      <div class="text-3xl mb-5">Chỉnh sửa đánh giá</div>
      <ReviewVEdit2 :data="data" />
    ...
    <div v-else>
      <div class="text-end">
        <div @click="openDialogSignin()" class="btn btn-success btn-sm">
      ...
      <ReviewVReview2
        :data="
          useReview.review.level != 'NaN' ? useReview.review : useReview.reviewDefault
        "
      />
      <!-- <ReviewVEdit2 :data="data" /> -->
    </div>
    ...
    <div v-else>
      <div v-for="(i, n) in dataPerPage" :key="i">
        <CommentsVCmt :data="i" />
        <div
          v-if="n < (dataPerPage.length > size ? size : dataPerPage.length) - 1"
          class="divider my-0"
        ></div>
      </div>
    </div>
