    ...
    <div v-else>
      <div class="space-y-3 mt-5" v-if="dataPerPage[0]">
        <div v-for="(i, n) in dataPerPage" :key="i._id">
          <DashboardVMonoTag :data="i" />
          <div v-if="n < useTag.List_tag.length - 1" class="divider"></div>
        </div>
      </div>
      <div v-else>
        <div class="text-center text-2xl my-10">Không có tag nào !?</div>
      </div>
    </div>
    <!-- btn chuyển trang -->
...
const maxPage = computed(() => {
  selectPage.value = 1;
  return Math.ceil(useTag.List_tag.length / size);
});
...
const dataPerPage = computed(() => {
  ...
  for (let i = 0; i < size; i++) {
    if (index < useTag.List_tag.length) list.push(useTag.List_tag[index]);
    index++;
  }
