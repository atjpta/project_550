<template>
  <div class="mt-5">
    <div class="flex justify-end">
      <nuxtLink to="/team/edit" class="btn btn-outline btn-success btn-sm lg:btn-md">
        tạo Nhóm
      </nuxtLink>
    </div>
    ...
    <div v-else>
      <div class="space-y-3 mt-5" v-if="dataPerPage[0]">
        <div v-for="(i, n) in dataPerPage" :key="i._id">
          <DashboardVMonoTeam :data="i" />
          <div v-if="n < useMember.List_myteam.length - 1" class="divider"></div>
        </div>
      </div>
      <div v-else>
        <div class="text-center text-2xl my-10">không có nhóm nào !?</div>
      </div>
    </div>
    <!-- btn chuyển trang -->
...
const maxPage = computed(() => {
  selectPage.value = 1;
  return Math.ceil(useMember.List_myteam.length / size);
});
...
const dataPerPage = computed(() => {
  ...
  for (let i = 0; i < size; i++) {
    if (index < useMember.List_myteam.length) list.push(useMember.List_myteam[index]);
    index++;
  }
