<template>
  ...
  >
    <div class="flex justify-between items-center">
      ...
      <div
        :data-tip="data.check ? 'Gỡ câu hỏi ' : 'Duyệt câu hỏi'"
        :class="data.check ? 'text-warning' : 'text-success'"
        class="font-medium flex tooltip tooltip-left lg:tooltip-top"
      >
        <div
          @click="checkQuestion()"
          :class="loadingCheck ? 'loading' : ''"
          class="h-10 sm:w-32 w-24 btn btn-sm btn-ghost justify-start"
        >
          {{ data.check ? "Gỡ câu hỏi" : "Duyệt câu hỏi" }}
        </div>
      </div>
      <!-- các nút chức năng -->
...
async function deleteModel(input) {
  ...
  try {
    const dataNotifi = {
      to: props.data.author[0]._id,
      content:
        `Câu hỏi "${props.data.title}" của bạn đã bị xóa ` + input ? `vì: ${input}` : "",
      type: "error",
    };
    ...
    useDialog.showDialog(
      {
        ...
        content: "Bạn muốn gỡ  bài viết này?",
        btn1: "Gỡ bài viết",
        btn2: "Hủy",
      },
      async () => {
        try {
          ...
          useAlert.setSuccess("Đã gỡ  thành công");
          await useRouteS.refreshData();
        } catch (error) {
          console.log(error);
        }
