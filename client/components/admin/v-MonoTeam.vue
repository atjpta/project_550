<template>
  ...
  >
    <div class="flex justify-between items-center">
      ...
      <div class="basis-1/3">
        <nuxt-link
          :to="`/user/${data.member[0].chief[0]._id}/overview`"
          class="flex items-center space-x-3 hover:text-info"
        >
          <div class="avatar">
            <div class="mask mask-squircle w-12 h-12">
              <img :src="data.member[0].chief[0].avatar_url" />
            </div>
          </div>
          <div>
            <div class="font-bold">{{ data.member[0].chief[0].name }}</div>
          </div>
        </nuxt-link>
      </div>
      <!-- tiêu đề -->
...
function showDataReport() {
  ...
  if (props.data.reports.length == 0) {
    useAlert.setInfo("Nhóm này không có báo cáo nào cả!!!!!!");
  }
}
...
async function deleteModel(input) {
  ...
  try {
    const dataNotifi = {
      to: props.data.member[0].chief[0]._id,
      content:
        `Nhóm "${props.data.name}" của bạn đã bị xóa ` + input ? `vì: ${input}` : "",
      type: "error",
    };
    await useTeam.deleteOne(props.data._id);
