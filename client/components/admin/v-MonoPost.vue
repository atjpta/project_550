...
function showDataReport() {
  ...
  if (props.data.reports.length == 0) {
    useAlert.setInfo("bài viết này không có báo cáo nào cả!!!!!!");
  }
}
...
async function warning(input) {
  ...
  try {
    const dataNotifi = {
      ...
      content: `bài viết "${props.data.title}" của bạn bị cảnh báo:  ${input}`,
      url: `/post/${props.data._id}`,
      type: "warning",
    };
...
async function deleteModel(input) {
  ...
  try {
    const dataNotifi = {
      to: props.data.author[0]._id,
      content: `bài viết "${props.data.title}" của bạn đã bị xóa vì:  ${input}`,
      type: "error",
    };
