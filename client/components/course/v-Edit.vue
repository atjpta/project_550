<template>
  <div>
    <div class="rounded-2xl bg-base-100 mx-auto">
      <div class="rounded-2xl bg-gradient-to-r from-indigo-500/10 via-purple-500/10 to-pink-500/10 p-5 item">
        <div class="text-center text-4xl font-extrabold">Chỉnh sửa môn học</div>
        ...
        <div>
          ...
          <input v-model="data.name" placeholder="nhập tên ..." type="text"
        ...
        <div>
          ...
          <select v-model="data.type" class="select-sm select select-primary w-full max-w-xs text-base">
            <option :value="{}" disabled>chọn</option>
            <option :value="i.id" :disabled="i.id == data.type" v-for="i in useTypeCourse.list" :key="i">
              {{ i.name }}
            </option>
          </select>
        </div>
        ...
        <div>
          ...
          <textarea v-model="data.introduce" placeholder="nhập tên ..." type="text"
