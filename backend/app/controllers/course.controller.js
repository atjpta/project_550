...
exports.findPerPageType = async (req, res, next) => {
    const { type, page, size } = req.params
    try {
        const document = await model.find({ type: type }).sort({ createdAt: -1 })
            .limit(size)
            .skip(size * (page - 1))
...
exports.maxLengthType = async (req, res, next) => {
    const { type } = req.params
    try {
        const document = (await model.find({ type: type }))
...
exports.findOneDelta = async (req, res, next) => {
    const { id } = req.params;
    const condition = {
        _id: id && mongoose.isValidObjectId(id) ? id : null,
    };
    ...
    try {
        const document = await model.findOne(condition).populate('type', 'name')
        if (!document) {
            return next(res.status(404).json({ Message: "không thể tìm thấy model" }));
        }
        return res.json(document);
